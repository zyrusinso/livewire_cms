<div class="divide-y divide-gray-800" x-data="{show: false}">
    <nav class="flex items-center bg-gray-900 px-3 py-2 shadow-lg hover:text-gray-200 focus:text-gray-200 focus:outline-none">
        <div>
        <button @click="show = !show" class="block h-8 mr-2 text-gray-400 items-center sm:hidden" >
        ...
        <div class="h-12 w-full flex items-center">
            <a href="{{ url('/') }}" class="w-full">
                <img src="{{ url('/img/logo.svg') }}" class="h-8">
            </a>
        </div>
        <div class="flex justify-end sm:w-8/12">
            <ul class="hidden sm:flex sm:text-left text-gray-200 text-xs">
                @foreach ($topNavLinks as $item)
                    <a href="{{ url('/'.$item->slug) }}"><li class="cursor-pointer px-4 py-2 hover:underline">{{ $item->label }}</li></a>
                @endforeach
            </ul>
        </div>
    </nav>
